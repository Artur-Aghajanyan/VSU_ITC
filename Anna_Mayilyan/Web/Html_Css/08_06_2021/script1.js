const data = [
 ...
 let total_ivest = data.filter(item => item.age <= 18).reduce((total,value) => total + value.investments, 0 )
 console.log(total_ivest);

 //Get array which will contain strings, and each string of the array will be contained the name and investments with the following format "Gevorg invested 180 $"
 let name_invest = data.map(item => item.name + " invested " + item.investments + "$")
 console.log(name_invest);

 //Get the array which will contain the top 3 investor's names
 let top_3 = data.sort((a,b) => b.investments-a.investments).slice(0,3).map(item => item.name);
 console.log(top_3);
